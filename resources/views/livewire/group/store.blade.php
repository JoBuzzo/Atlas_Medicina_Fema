<div>
    <form wire:submit.prevent='saveGroup'>
        ...
        @error('selected_categories')
            <span
                class="block mt-2 text-xs font-medium text-red-600 lg:inline-block dark:text-red-400">{{ $message }}</span>
        @enderror
    ...
    <button wire:click='resetImages'>Adicionar mais imagens</button>
    <form wire:submit.prevent='saveImage'>

        <input type="text" wire:model='titleImage' class="text-black">
        @error('titleImage')
            {{ $message }}
        @enderror

        <input type="file" wire:model='image' class="text-black">
        @error('image')
            {{ $message }}
        @enderror

        <input type="text" wire:model='description' class="text-black">
        @error('description')
            {{ $message }}
        @enderror
